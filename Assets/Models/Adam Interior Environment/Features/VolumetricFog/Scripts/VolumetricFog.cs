...
public class VolumetricFog : MonoBehaviour
{
	...
	bool m_HardwareRequirementsChecked = false;
	bool m_Supported = true;

	bool CheckHardwareRequirements()
	{
		if (m_HardwareRequirementsChecked)
			return m_Supported;

		m_Supported = SystemInfo.supportsComputeShaders;

		if (!m_Supported)
		{
			enabled = false;
			Debug.LogWarning("VolumetricFog requires compute shaders to run, disabling.");
		}

		m_HardwareRequirementsChecked = true;
		return m_Supported;
	}
}
